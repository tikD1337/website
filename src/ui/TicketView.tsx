        ...
        <div className="section">
          ...
          {asks.map(a => (
            <div key={a.id} className="ask">
              ...
              {/*
                Просьба остаётся доступной и после того, как о ней
                попросили.

                «Выйдите и войдите заново» — обычное действие первой
                линии, и попросить об этом до того, как найдена
                причина, естественно. Заявитель честно выполнит и
                скажет, что ничего не изменилось: перевыпускать в
                билете пока нечего. Выключенная кнопка означала бы,
                что одна разумная попытка закрыла единственный путь к
                решению навсегда — сценарий становился непроходимым.
              */}
              <button
                className="act"
                type="button"
              ...
              >
                {session.askedFor.includes(a.id)
                  ? 'Попросить ещё раз'
                  : 'Попросить'}
              </button>
            </div>
