...
export function defaultConfig(): DialogueConfig {
  return {
    ...
    apiKey: '',
    /*
      Минута, а не двадцать секунд.

      Найдено живой проверкой: первый запрос к Ollama занял сорок
      секунд — модель грузится в видеопамять, — а последующие
      укладывались в секунду. При прежнем таймауте самая первая реплика
      разговора неизбежно падала, размыкатель отключал модель, и техник
      видел плашку «модель не ответила» на совершенно исправной
      установке.
    */
    timeoutMs: 60_000,
    speak: false,
