...
export function gradeNote(
  ...
  */
  const GUI_EVIDENCE = [
    ...
    /*
      Срез 3: каталог. Список застыл на предыдущем срезе, и заметка,
      честно описывающая разбор через карточки и членство, теряла балл
      ни за что — при том что сами цели сценария предлагают этот путь.
    */
    'карточк', 'консоли каталога', 'консоль каталога',
